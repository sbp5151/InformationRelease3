package com.jld.InformationRelease.view.login_register.countryCode;
...
public class SearchEngine {
					...
					if (hanzi2Pinyin == null || hanzi2Pinyin.size() <= 0) {
						try {
							int resId = context.getResources().getIdentifier(
									DB_FILE, "raw", context.getPackageName());
							if (resId <= 0) {
	...
	private static class SearchIndex {
		...
		private void createPinyinList(HashMap<String, Object> hanzi2Pinyin) {
			...
			if (hanzi2Pinyin != null && hanzi2Pinyin.size() > 0) {
				...
				for (char c : cArr) {
                    ...
                    ArrayList<HashMap<String, Object>> yins = (ArrayList<HashMap<String, Object>>) hanzi2Pinyin
							.get(s);
		...
		private void toPinyinArray(String base, String firstLatter,
                                   HashSet<String> pyRes, HashSet<String> flRes,
                                   ArrayList<String[]> pys) {
			if (pys.size() > 0) {
				String[] py = pys.get(0);
				...
				for (String y : py) {
					if (y.length() > 0) {
						toPinyinArray(base + y, firstLatter + y.charAt(0),
								pyRes, flRes, cpys);
					} else {
